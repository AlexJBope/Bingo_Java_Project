package com.example.bingoboard;//package com.example.bingoboard;
...
public class BingoBoard {
    int[] numbs = new int[25];  //declarations
    boolean[] marks = new boolean[25];
    String ID;

    public BingoBoard(String id){ //takes in name for board (was not actually used)
        ID = id;
        ...
        for(int i = 0; i < 100; i++){   //randomly mixes an array (r) of index values 0-15
        ...
        for(int i = 0; i < 5; i++){numbs[i] = r1[r[i]];}    //fills the objects array numbs with the values for the bingo board
        ...
        //visualization for help with coding
    ...
    // display ID / name
    public String getId(){return ID;}       //Gives ability to change id or return id
    public void setID(String ID){this.ID = ID;}

    //add a marker to a board
    public void AddMarker(int BingoNumber){         //marks array stores boolean values to store whether a position has a marker or not
    ...
    public void ClearBoard(){           //clears all markers off of board
        Arrays.fill(marks, Boolean.FALSE);
        marks[12] = true;
    }

    //Diplay bingo card in line
    public void showBoard(){        //displays bingo board in command line
